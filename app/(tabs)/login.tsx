...
  // [수정] '일' 비중을 조절하기 위해 일부 가벼운 활동을 다시 '휴식'으로 변경
  const scheduleTemplates: Record<string, any[]> = {
    // 1~5: 평일 (과도한 중복)
    Weekday: [
      // --- 오전 중복 클러스터 (3개) ---
      { start: toMin(9, 45), end: toMin(11, 15), type: '자기개발', action: '노동', purpose: '메인 프로젝트' }, // 90m
      { start: toMin(10, 0), end: toMin(12, 0), type: '자기개발', action: '수업', purpose: 'GCP 자격증' }, // 120m
      { start: toMin(10, 30), end: toMin(11, 45), type: '휴식', action: '오락', purpose: '학술지 탐방' }, // 75m (휴식으로 복귀)
      
      // --- 오후 중복 클러스터 (3개) ---
      { start: toMin(14, 30), end: toMin(16, 0), type: '자기개발', action: '노동', purpose: '백엔드 API 구현' }, // 90m
      { start: toMin(15, 0), end: toMin(16, 30), type: '자기개발', action: '노동', purpose: '코딩 풀이' }, // 90m (자기개발 유지)
      { start: toMin(15, 45), end: toMin(17, 15), type: '자기개발', action: '운동', purpose: 'DevOps 스터디' }, // 90m

      // --- 저녁 중복 클러스터 (3개) ---
      { start: toMin(21, 0), end: toMin(22, 30), type: '자기개발', action: '노동', purpose: '오픈 프로젝트' }, // 90m (자기개발 유지)
      { start: toMin(21, 15), end: toMin(23, 0), type: '자기개발', action: '노동', purpose: 'Swift 튜토리얼' }, // 105m
      { start: toMin(22, 0), end: toMin(23, 30), type: '휴식', action: '수면', purpose: '프로그래밍 청취' }, // 90m (휴식으로 복귀)
    ],
    // 6, 0: 주말 (과도한 중복)
    Weekend: [
      // --- 오전 중복 클러스터 (3개) ---
      { start: toMin(9, 30), end: toMin(11, 0), type: '자기개발', action: '수업', purpose: '알고리즘' }, // 90m
      { start: toMin(10, 0), end: toMin(11, 30), type: '휴식', action: '오락', purpose: '데이터셋 탐색' }, // 90m (휴식으로 복귀)
      { start: toMin(10, 15), end: toMin(11, 45), type: '자기개발', action: '노동', purpose: '기술 아티클' }, // 90m
      
      // --- 오후 중복 클러스터 (3개) ---
      { start: toMin(14, 0), end: toMin(16, 30), type: '자기개발', action: '노동', purpose: '토이 프로젝트' }, // 150m (자기개발 유지)
      { start: toMin(15, 0), end: toMin(17, 0), type: '자기개발', action: '운동', purpose: '언어 입문서' }, // 120m
      { start: toMin(16, 0), end: toMin(17, 30), type: '자기개발', action: '노동', purpose: '백엔드' }, // 90m (자기개발 유지)

      // --- 저녁 중복 클러스터 (3개) ---
      { start: toMin(20, 30), end: toMin(22, 30), type: '자기개발', action: '수업', purpose: '엔진 튜토리얼' }, // 120m (자기개발 유지)
      { start: toMin(21, 0), end: toMin(22, 45), type: '자기개발', action: '노동', purpose: '리팩토링' }, // 105m
      { start: toMin(22, 15), end: toMin(23, 45), type: '자기개발', action: '수업', purpose: '프레임워크' }, // 90m (자기개발 유지)
    ],
  };
...
          action: task.action,
          purpose: task.purpose,
          // [수정] '운동' 또는 '오락'을 목표로 설정
          isGoal: task.action === '운동' || task.action === '오락', 
          fix: false,
          createdAt: serverTimestamp(),
        });
      }

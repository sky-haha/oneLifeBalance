...
/**
 * [수정] 2025-10-20부터 2025-11-19까지 '여가 과다 / 일 부족' 테스트 데이터를 생성합니다.
...
const seedMy30DayData = async (uid: string) => {
  ...
  console.log("30일 '여가 과다' 데이터 생성을 시작합니다...");
  Alert.alert(
    "데이터 생성 시작",
    "여가가 많고 일이 적은 30일치 데이터를 Firestore에 쓰는 중입니다. (약 10-15초 소요)"
  );

  const toMin = (h: number, m: number = 0) => h * 60 + m;

  // 1. [수정] 요일별 일정 템플릿 (여가 극대화)
  const scheduleTemplates: Record<string, any[]> = {
    // 1~5: 평일 (W: 3.5시간, L: 8.5시간)
    Weekday: [
      { start: toMin(9, 30), end: toMin(10, 0), type: '이동', action: '기타', purpose: '가까운 카페로 이동' }, // (W)
      { start: toMin(10), end: toMin(12, 30), type: '자기개발', action: '노동', purpose: '오전 작업 (재택)' }, // (W)
      { start: toMin(14), end: toMin(14, 30), type: '이동', action: '기타', purpose: '복귀' }, // (W)
      // --- 여가 시간 ---
      { start: toMin(14, 30), end: toMin(16, 30), type: '휴식', action: '오락', purpose: '게임' }, // (L)
      { start: toMin(16, 0), end: toMin(17, 30), type: '자기개발', action: '운동', purpose: '요가' }, // (L) (겹침)
      { start: toMin(17, 30), end: toMin(19, 0), type: '휴식', action: '수면', purpose: '낮잠' }, // (L) -> '수면'이지만 type이 '휴식'이라 여가로 집계됨
      { start: toMin(20, 30), end: toMin(23, 59), type: '휴식', action: '오락', purpose: '영화 시청' }, // (L)
    ],
    // 6, 0: 주말 (W: 1시간, L: 8.5시간)
    Weekend: [
      { start: toMin(10), end: toMin(11), type: '자기개발', action: '수업', purpose: '주말 온라인 강의' }, // (W)
      // --- 여가 시간 ---
      { start: toMin(9, 30), end: toMin(10, 30), type: '자기개발', action: '운동', purpose: '아침 조깅' }, // (L) (겹침)
      { start: toMin(14), end: toMin(17), type: '휴식', action: '오락', purpose: '친구와 게임' }, // (L)
      { start: toMin(16, 30), end: toMin(17, 30), type: '휴식', action: '수면', purpose: '낮잠' }, // (L) (겹침)
      { start: toMin(20, 30), end: toMin(23, 59), type: '휴식', action: '오락', purpose: '주말 예능 시청' }, // (L)
    ],
  };

  // 2. [수정] 공통 일정 (매일) - '필수' 항목 (W: 0, L: 0)
  const commonSchedules = [
    { start: toMin(0), end: toMin(8, 0), type: "개인", action: "수면", purpose: "수면" },
    { start: toMin(8, 0), end: toMin(8, 45), type: "개인", action: "기타", purpose: "기상 및 준비" },
    { start: toMin(8, 45), end: toMin(9, 30), type: "식사", action: "기타", purpose: "아침 식사" },
    { start: toMin(12, 30), end: toMin(14, 0), type: "식사", action: "기타", purpose: "점심 식사" },
    { start: toMin(19, 0), end: toMin(20, 30), type: "식사", action: "기타", purpose: "저녁 식사" },
  ];
